...
You're building a post-default debt collections system with three AI agents operating behind a single continuous borrower experience. The borrower interacts with what feels like one system. Behind the scenes, a Temporal (temporal.io) workflow orchestrates handoffs between three specialized agents, two over chat and one over voice. Each agent has a self-learning loop that autonomously improves its own performance, and the loop itself evolves its own evaluation methodology over time.
